	...
	if (cropTopPercent + cropBottomPercent >= 100) {
		...
		if (adjustingElement === 'cropTopPercent') {
			...
			if (cropTopPercent > maxTop) {
				...
				console.warn(`顶部裁剪已调整到最大可设置值: ${cropTopPercent}% (底部固定为 ${cropBottomPercent}%)`);
			}
			...
			if (cropBottomPercent > maxBottom) {
				...
				console.warn(`底部裁剪已调整到最大可设置值: ${cropBottomPercent}% (顶部固定为 ${cropTopPercent}%)`);
			}
		} else {
			// 程序初始化或其他情况，按比例调整
			...
			elements.cropTop.value = cropTopPercent.toString();
			elements.cropBottom.value = cropBottomPercent.toString();
			console.warn(`裁剪参数已按比例调整: 顶部 ${cropTopPercent}%, 底部 ${cropBottomPercent}%`);
		}
	}
...
async function renderPreview() {
	...
	try {
		...
		// 显示加载状态
		previewContainer.innerHTML = '<div style="text-align: center; padding: 20px; color: #666;">正在生成预览...</div>';
